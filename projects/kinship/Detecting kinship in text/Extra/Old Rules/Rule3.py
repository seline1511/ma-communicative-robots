...
def find_relation3(utterances, kinship_term, character_list):
    '''
    ...
    for k in range(len(tokens)-1):
        if tokens[k]== "your":
            if tokens[k+1] in ["brother", "brothers", "sister", "sisters", "cousin", "cousins", "mother", "father", "mom", "dad", "son", "sons", "daughter", "daughters", "neice", "nephew", "twin", "aunt", "uncle", "child", "children", "parent", "parents"]:
                ...
                relation = None
                # Heuristic: source = source of next utterance, or of previous utterance
                #     if next utterance doesn't exist
            if next_utterance:
                source = next_utterance["source"]
            else:
                source = prev_utterance["source"]
...
def main(data):
    ...
    for kinship_term, utterances in train_data.items():
        for utterance_set in utterances:
            source, relation, target = find_relation3(utterance_set, kinship_term, character_list)
            i+=1
            if source != None:
